...
class ItineraryApp {
  ...
  addLocation() {
    ...
    const dup = this.locations.find(
      (l, i) =>
        i !== this.editingLocationIndex &&
        (l.name || "").trim().toLowerCase() === lower
    );
    if (dup)
      return this.showMessage(
        "Tên địa điểm đã tồn tại trong danh sách. Vui lòng nhập tên khác.",
        "error"
      );
    ...
    if (
      ...
      this.editingLocationIndex < this.locations.length
    ) {
    ...
    if (
      this.isEditing &&
      this.el.detailContent &&
      this.el.detailPanel &&
      this.el.detailPanel.classList.contains("show")
    ) {
      this.renderDetailContent(
        Object.assign({}, this.currentTrip || {}, {
  ...
  validateTrip(trip) {
    if (!trip.name || !trip.name.trim())
      return { ok: false, msg: "Tên chuyến đi là trường bắt buộc" };
    if (!trip.start || !trip.start.trim())
      return { ok: false, msg: "Điểm đi là trường bắt buộc" };
    if (!trip.startDate || !trip.startDate.trim())
      return { ok: false, msg: "Ngày bắt đầu là trường bắt buộc" };
    if (!trip.startTime || !trip.startTime.trim())
      return { ok: false, msg: "Giờ bắt đầu là trường bắt buộc" };
    if (!trip.end || !trip.end.trim())
      return { ok: false, msg: "Điểm đến là trường bắt buộc" };
    if (!trip.endDate || !trip.endDate.trim())
      return { ok: false, msg: "Ngày kết thúc là trường bắt buộc" };
    if (!trip.endTime || !trip.endTime.trim())
      return { ok: false, msg: "Giờ kết thúc là trường bắt buộc" };
    // kiểm tra rằng ngày giờ kết thúc phải sau ngày giờ bắt đầu
    try {
      const sDate = new Date(
        trip.startDate + "T" + (trip.startTime || "00:00")
      );
      ...
      if (eDate.getTime() <= sDate.getTime())
        return {
          ok: false,
          msg: "Ngày kết thúc phải lớn hơn ngày/giờ bắt đầu",
        };
    } catch (e) {}
    return { ok: true };
  }
  ...
  updateActionButtons() {
    ...
    try {
      if (this.el.deleteBtn) {
        if (this.currentTrip) this.el.deleteBtn.classList.remove("hidden");
        else this.el.deleteBtn.classList.add("hidden");
      }
      // nút xóa trên panel
      if (this.el.deleteBtnPanel) {
        if (this.currentTrip) this.el.deleteBtnPanel.classList.remove("hidden");
        else this.el.deleteBtnPanel.classList.add("hidden");
      }
    } catch (e) {}
  }
  ...
  renderSavedList() {
    ...
    items.forEach((t, i) => {
      ...
      const dateLabel = t.createdAt
        ...
        : t.start || "";
      li.innerHTML = ` <div class="saved-item-name">${
        t.name || "Chuyến đi #" + (i + 1)
      }</div>
      ...
      if (
        this.currentTrip &&
        t.createdAt &&
        this.currentTrip.createdAt === t.createdAt
      ) {
        li.classList.add("active");
      }
      box.appendChild(li);
  ...
  loadTrip(t) {
    ...
    this.el.startLocation &&
      (this.el.startLocation.value = t.startLocation || "");
    ...
    try {
      if (this.el.tripName) {
        ...
        this.el.tripName.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }
  ...
  renderDetailContent(t, editable = false) {
    ...
    if (!editable) {
      ...
      const locHtml = (t.locations || [])
        .map(
          (l, i) => `<li class="location-item">
        <div><strong>${i + 1}. ${l.name}</strong></div>
        <div>
          ${l.dateStart || ""} ${l.arrival || ""} - ${l.dateEnd || ""} ${
            l.departure || ""
          }
        </div>
      </li>`
        ...
        .join("");
      content.innerHTML = `
        <div class="detail-block detail-header-block"><h3 class="detail-title">${
          t.name || ""
        }</h3></div>
        <div class="detail-block detail-grid">
          <div class="detail-row"><div class="label">Điểm đi</div><div class="value">${
            t.startLocation || ""
          }</div></div>
          <div class="detail-row"><div class="label">Ngày bắt đầu</div><div class="value">${
            t.startDate || ""
          } ${t.startTime || ""}</div></div>
          <div class="detail-row"><div class="label">Điểm đến</div><div class="value">${
            t.endLocation || ""
          }</div></div>
          <div class="detail-row"><div class="label">Ngày kết thúc</div><div class="value">${
            t.endDate || ""
          } ${t.endTime || ""}</div></div>
        </div>
        <div class="detail-block">
          <h4>Các điểm tham quan</h4>
          <ul class="location-list locations-vertical">${
            locHtml || "<li>Chưa có điểm tham quan</li>"
          }</ul>
        </div>
      `;
    } else {
      // hiển thị biểu mẫu có thể chỉnh sửa bên trong bảng điều khiển
      const locFields = (t.locations || [])
        .map(
          (l, i) => `
        <div class="panel-loc-block" data-idx="${i}">
          ...
          <input class="panel-loc-name" data-idx="${i}" value="${
            l.name || ""
          }" style="border:1px solid #aaa;padding:4px;border-radius:4px;" />
          <div style="display:flex;gap:8px;margin-top:6px;">
            <input type="date" class="panel-loc-date-start" data-idx="${i}" value="${
            l.dateStart || ""
          }" style="border:1px solid #aaa;padding:4px;border-radius:4px;" placeholder="Ngày bắt đầu" />
            <input type="date" class="panel-loc-date-end" data-idx="${i}" value="${
            l.dateEnd || ""
          }" style="border:1px solid #aaa;padding:4px;border-radius:4px;" placeholder="Ngày kết thúc" />
          </div>
          <div style="display:flex;gap:8px;margin-top:6px;">
            <input type="time" class="panel-loc-arrival" data-idx="${i}" value="${
            l.arrival || ""
          }" style="border:1px solid #aaa;padding:4px;border-radius:4px;" />
            <input type="time" class="panel-loc-departure" data-idx="${i}" value="${
            l.departure || ""
          }" style="border:1px solid #aaa;padding:4px;border-radius:4px;" />
          </div>
        </div>
      `
        ...
        .join("");
      content.innerHTML = `
        <div class="form-group"><label>Tên chuyến đi</label><input id="panel-trip-name" value="${
          t.name || ""
        }" /></div>
        <div class="form-group"><label>Điểm đi</label><input id="panel-start-location" value="${
          t.startLocation || ""
        }" /></div>
        <div class="form-group"><label>Ngày bắt đầu</label><input id="panel-start-date" type="date" value="${
          t.startDate || ""
        }" /></div>
        <div class="form-group"><label>Giờ bắt đầu</label><input id="panel-start-time" type="time" value="${
          t.startTime || ""
        }" /></div>
        <div class="form-group"><label>Điểm đến</label><input id="panel-end-location" value="${
          t.endLocation || ""
        }" /></div>
        <div class="form-group"><label>Ngày kết thúc</label><input id="panel-end-date" type="date" value="${
          t.endDate || ""
        }" /></div>
        <div class="form-group"><label>Giờ kết thúc</label><input id="panel-end-time" type="time" value="${
          t.endTime || ""
        }" /></div>
        <hr />
      ...
      Array.from(content.querySelectorAll(".btn-panel-loc-remove")).forEach(
        (btn) => {
          btn.addEventListener("click", (e) => {
            const idx = parseInt(btn.dataset.idx, 10);
      ...
      Array.from(content.querySelectorAll(".btn-edit-loc-panel")).forEach(
        (btn) => {
          btn.addEventListener("click", (e) => {
            const idx = parseInt(btn.dataset.idx, 10);
            if (idx >= 0 && idx < this.locations.length) {
  ...
  enableEditPanel() {
    if (!this.currentTrip) {
      this.showMessage("Vui lòng chọn lịch để chỉnh sửa", "error");
      return;
    }
    ...
    this.locations = JSON.parse(
      JSON.stringify(this.currentTrip.locations || [])
    );
    this.renderDetailContent(
      Object.assign({}, this.currentTrip, { locations: this.locations }),
      true
    ...
    this.isEditing = true;
    this.showMessage(
      "Chỉnh sửa trong khung chi tiết, sửa xong nhấn Lưu.",
      "success"
    );
    this.updateActionButtons();
  }
  ...
  saveEditPanel() {
    ...
    try {
      ...
      updated.startLocation =
        content.querySelector("#panel-start-location")?.value ||
        updated.startLocation;
      updated.startDate =
        content.querySelector("#panel-start-date")?.value || updated.startDate;
      updated.startTime =
        content.querySelector("#panel-start-time")?.value || updated.startTime;
      updated.endLocation =
        content.querySelector("#panel-end-location")?.value ||
        updated.endLocation;
      updated.endDate =
        content.querySelector("#panel-end-date")?.value || updated.endDate;
      updated.endTime =
        content.querySelector("#panel-end-time")?.value || updated.endTime;
      ...
      updated.locations = locNames.map((el) => {
        ...
        return {
          name: (el.value || "").trim(),
          dateStart:
            content.querySelector(`.panel-loc-date-start[data-idx="${idx}"]`)
              ?.value || "",
          dateEnd:
            content.querySelector(`.panel-loc-date-end[data-idx="${idx}"]`)
              ?.value || "",
          arrival:
      ...
      for (let i = 0; i < updated.locations.length; i++) {
        ...
        if (!loc.name || !loc.name.trim())
          return this.showMessage(
            `Vui lòng nhập tên cho địa điểm thứ ${i + 1}`,
            "error"
          );
        ...
        if (isNaN(locStart.getTime()) || isNaN(locEnd.getTime())) {
          return this.showMessage(
            `Ngày bắt đầu/kết thúc của địa điểm thứ ${i + 1} không hợp lệ`,
            "error"
          );
        }
        if (
          locStart.getTime() < tripStart.getTime() ||
          locStart.getTime() > locEnd.getTime()
        ) {
          return this.showMessage(
            `Ngày bắt đầu của địa điểm thứ ${
              i + 1
            } phải lớn hơn hoặc bằng ngày bắt đầu chuyến đi và nhỏ hơn ngày kết thúc của địa điểm`,
            "error"
          );
        }
        if (
          locEnd.getTime() > tripEnd.getTime() ||
          locEnd.getTime() < locStart.getTime()
        ) {
          return this.showMessage(
            `Ngày kết thúc của địa điểm thứ ${
              i + 1
            } phải nhỏ hơn hoặc bằng ngày kết thúc chuyến đi và lớn hơn ngày bắt đầu của địa điểm`,
            "error"
          );
        }
      ...
      const v = this.validateTrip({
        name: updated.name,
        start: updated.startLocation,
        startDate: updated.startDate,
        startTime: updated.startTime,
        end: updated.endLocation,
