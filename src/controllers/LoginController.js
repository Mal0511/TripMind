...
let getLoginController = async (req, res) => {
    ...
    res.json({ message: "Login successful", userType: "user" });
...
let getRegisterController = async (req, res) => {
    try {
        const { fullname: fullName, regUsername, password, email, phone } = req.body;
        const existUser = await User.findOne({ where: {  username: regUsername  } }); 
        if (existUser) {
